...
from m2models.common.utils import load_state_dict, save_checkpoint
from m2models.modules.evaluator import Evaluator
from m2models.modules.exponential_moving_average import (
    ExponentialMovingAverage,
)
from m2models.modules.loss import AtomwiseL2Loss, DDPLoss, L2MAELoss
from m2models.modules.normalizer import Normalizer
from m2models.modules.scaling.compat import load_scales_compat
from m2models.modules.scaling.util import ensure_fitted
from m2models.modules.scheduler import LRScheduler
...
class BaseTrainer(ABC):
    ...
    def precompute_dgl_dataset(self, config, dataset_type):
        # precompute dgl data from pyg data
        # note that new data still uses pyg format
        # get dgl g and lg using data.g and data.lg
        # return if dgl is already pre-computed
        dataset = registry.get_dataset_class(
            config["task"]["dataset"])(config[dataset_type])

        parser = config[dataset_type]["src"].split('/')
        head_dir = "/".join(parser[:-2])
        dgl_path = head_dir + '/' + parser[-2] + '_dgl/'
        os.makedirs(dgl_path, exist_ok=True)
        config[dataset_type]["src"] = dgl_path
        # if dgl dataset is already pre-computed, just return its path
        try:
            dgl_dataset = registry.get_dataset_class(
                config["task"]["dataset"])(config[dataset_type])
            print (len(dgl_dataset), len(dataset))
            assert len(dgl_dataset) == len(dataset), "LMDB for dgl dataset is not complete"
        except AssertionError:
            from m2models.models.alignn.pyg2dgl import convert2dgl_precomputed
            import pickle 
            db = lmdb.open(
                    dgl_path+'data.lmdb',
                    # TODO: maybe need modification here
                    map_size=1099511627776 * 4,
                    subdir=False,
                    meminit=False,
                    map_async=True)

            pbar = tqdm(total=len(dataset), desc="Preprocessing data into LMDBs")

            idx = 0
            for sample in dataset:
                natoms = sample.natoms
                sample.natoms=torch.tensor([natoms])
                (
                    edge_index,
                    distances,
                    distance_vec,
                    ...
                    neighbors,
                ) = self.generate_graph(data=sample, 
                                        cutoff=self.config["model_attributes"].get("cutoff", 6.0), 
                                        max_neighbors=self.config["model_attributes"].get("max_neighbors", 50), 
                                        use_pbc=self.config["model_attributes"].get("use_pbc", True),
                                        otf_graph=self.config["model_attributes"].get("otf_graph", None)
                                        )
                sample.natoms=int(sample.natoms[0])
                sample.edge_index = edge_index
                sample.distances = distances
                sample.distance_vec = distance_vec
                sample.cell_offsets = cell_offsets

                data_object = convert2dgl_precomputed(sample)
    ...
    # determine whether to use dgl data format
    # to use pre-computed dgl data, **otf_graph need to be False**
    # otherwise, pyg data is converted to dgl data after computing edges
    def prepare_dgl_dataset(self, config):
        train_dgl_path = self.precompute_dgl_dataset(self.config, "dataset")
        self.config["dataset"]["src"] = train_dgl_path
        if config.get("val_dataset", None):
            val_dgl_path = self.precompute_dgl_dataset(self.config, "val_dataset")
            self.config["val_dataset"]["src"] = val_dgl_path
        if config.get("test_dataset", None):
            test_dgl_path = self.precompute_dgl_dataset(self.config, "test_dataset")
            self.config["test_dataset"]["src"] = test_dgl_path
        return
    ...
    def load_loss(self):
        ...
        self.loss_fn["dist"] = self.config["optim"].get("loss_dist", "mae")
        self.loss_fn["classification"] = self.config["optim"].get("loss_classification", "ce")
        ...
        for i, batch in tqdm(
            ...
            if self.name != 'classification':
                ...
                with torch.cuda.amp.autocast(enabled=self.scaler is not None):
                    ...
                    readout = torch.nn.Softmax(dim=-1)
                    prediction = readout(out["energy"])
                    prediction = prediction.argmax(dim=-1)
                    out_all.append(prediction)
                    batch_all.append(torch.cat([sample.y for sample in batch]))

        if self.name == 'classification':
            out_all = torch.cat(out_all)
            batch_all = torch.cat(batch_all)
            metrics = self._compute_metrics(out_all, batch_all, evaluator, metrics)
