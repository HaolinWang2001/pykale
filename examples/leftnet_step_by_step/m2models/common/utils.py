...
def _get_project_root():
    ...
    root_folder = registry.get("m2models_root", no_warning=True)

    if root_folder is not None:
        assert isinstance(root_folder, str), "m2models_root must be a string"
...
def setup_imports(config: Optional[dict] = None):
    ...
    try:
        ...
        importlib.import_module("m2models.common.logger")

        import_keys = ["trainers", "datasets", "models", "tasks"]
        for key in import_keys:
            for f in (project_root / "m2models" / key).rglob("*.py"):
                _import_local_file(f, project_root=project_root)
...
def new_trainer_context(*, config: Dict[str, Any], args: Namespace):
    ...
    if TYPE_CHECKING:
        from m2models.tasks.task import BaseTask
        from m2models.trainers import BaseTrainer
